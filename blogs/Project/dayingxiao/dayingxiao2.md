...
- 第二步；对库存的扣减，使用 decr + lock 锁的方式（兜底）进行处理。高并发场景下的 Redis 的原子操作 DECR 和分布式锁作为兜底方案。
...
我们在过往的开发中，我们提供了活动装配、策略装配，所以要单独为装配提供好一个接口来使用。
之后，串联抽奖所需的过程模块，从参数校验、参与活动、抽奖策略、存放结果、更新订单，直至最终返回抽奖结果。
![img_20.png](img_20.png)
- 串联整个抽奖流程，提供 API
- 提供以活动为主导的，预热装配动作
- 抽奖策略模块中，校验账户额度。【之前的一个策略规则，需要根据已经抽奖次数进行解锁】
...
延迟队列消费更新数据库做优化，如果数据库库存已经清空，则不更新，减少数据库操作。给三个redis延迟队列的相关方法加了sku参数用来区分。
...
### 用户行为返利结算
将接收MQ消息开始结算返利。这里也就是给用户的活动账户充值。并提供一个日历签到返利的接口，用于后续对接到前端UI使用。
